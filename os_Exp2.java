...
class buffer_pool {
	public static final int M = 10; // 定义生产者数量为10个
	public static final int K = 10; // 定义消费者数量为10个
...
public class Exp extends buffer_pool {
	...
	public static void main(String[] args) {
		...
		System.out.println("已创建"+M+"个生产者!");
		consumer[] c = new consumer[K];
		for (int i = 0; i < K; i++)
			c[i] = new consumer((char) ('a' + i));
		System.out.println("已创建"+K+"个消费者!");
		
		// 生产者、消费者各自进入就绪队列
		for (int i = 0; i < M; i++)
			producer.ready(p[i]);
		System.out.println(M+"个生产者已就绪!");
		for (int i = 0; i < K; i++)
			consumer.ready(c[i]);
		System.out.println(K+"个消费者已就绪!");
		printline();
